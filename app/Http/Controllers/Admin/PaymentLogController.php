...
class PaymentLogController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$paymentRecord) {
            $paymentRecord = deposit::selectRaw('
                COUNT(id) AS totalPaymentLog,
                COUNT(CASE WHEN status = 1 THEN id END) AS paymentSuccess,
                (COUNT(CASE WHEN status = 1 THEN id END) / COUNT(id)) * 100 AS paymentSuccessPercentage,
                COUNT(CASE WHEN status = 2 THEN id END) AS pendingPayment,
                (COUNT(CASE WHEN status = 2 THEN id END) / COUNT(id)) * 100 AS pendingPaymentPercentage,
                COUNT(CASE WHEN status = 3 THEN id END) AS cancelPayment,
                (COUNT(CASE WHEN status = 3 THEN id END) / COUNT(id)) * 100 AS cancelPaymentPercentage,
                COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN id END) AS todayPayment,
                (COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN id END) / COUNT(id)) * 100 AS todayPaymentPercentage
            ')
                ->get()
                ->toArray();
    ...
    public function search(Request $request)
    {
        ...
        return DataTables::of($deposit)
            ...
            ->addColumn('amount', function ($item) {
                ...
                return "<h6 class='mb-0 $statusClass '>" . currencyPosition(getAmount($item->payable_amount_in_base_currency)) . "</h6>";
            })
            ...
            ->addColumn('payable', function ($item) {
                return "<h6>".getAmount($item->payable_amount).' '. $item->payment_method_currency."</h6>";
            })
            ...
            ->addColumn('action', function ($item) {
                ...
                if (optional($item->gatewayable)->id > 999) {
                ...
                data-method='" . optional($item->gatewayable)->name . "'
                data-gatewayimage='" . getFile(optional($item->gatewayable)->driver, optional($item->gatewayable)->image) . "'
        ...
        return DataTables::of($funds)
            ...
            ->addColumn('method', function ($item) {
                return '<a class="d-flex align-items-center me-2 cursor-unset" href="javascript:void(0)">
                                ...
                                <div class="flex-grow-1 ms-3">
                                  <h5 class="text-hover-primary mb-0">' . optional($item->gatewayable)->name . '</h5>
                                </div>
                              </a>';
            })
            ->addColumn('amount', function ($item) {
            ...
            ->addColumn('action', function ($item) {
                ...
                data-amount='" . getAmount($item->payable_amount).' '. $item->payment_method_currency . "'
                data-method='" . optional($item->gatewayable)->name . "'
                data-gatewayimage='" . getFile($item->gatewayable->driver, $item->gatewayable->image) . "'
                data-datepaid='" . dateTime($item->created_at) . "'
                data-status='$item->status'
                data-username='" . optional($item->user)->username . "'
                data-action='" . route('admin.payment.action', $item->id) . "'
