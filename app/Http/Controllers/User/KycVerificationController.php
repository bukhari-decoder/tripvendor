...
class KycVerificationController extends Controller
{
    ...
    {
        $kyc = Kyc::where('id', $request->kycType)->where('status', 1)->where('apply_for', 0)->firstOrFail();
    ...
    public function verificationHistory(Request $request){
        ...
        return view(template().'user.profile.partials.kyc_history', $data);
    }
    public function kycFormDetails(Request $request)
    {
        $val = $request->input('kycTypeID');
        $kyc = Kyc::where('id', $val)->where('apply_for', 0)->firstOr(function () {
            throw new \Exception('The kyc was not found.');
        });
        $oldKyc = UserKyc::where('kyc_id', $kyc->id)->where('user_id', Auth::user()->id)->latest()->first();
